...
def hash_video_id(video_id: str) -> int:
    ...
    Returns:
        Unique hash integer derived from the video ID.
    """
...
def assign_split_from_hashmod(hashmod10: int) -> str:
    ...
    Args:
        hashmod10: Hash of the video ID modulo 10.
...
def validate_manifest(samples: pd.DataFrame) -> None:
    ...
    Args:
        samples: DataFrame containing the manifest data.
...
def split_manifest(manifest_path: str | Path, output_dir: str | Path) -> dict[str, Path]:
    ...
    Args:
        ...
        output_dir: Directory where the split manifest files will be saved.

    Returns:
        Mapping from split names to their corresponding manifest file paths.
    ...
    samples["video_id_hashmod10"] = samples["video_id"].map(lambda vid: hash_video_id(vid) % 10)
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for manifest splitting.

    Returns:
        Parsed arguments containing the input manifest and output directory.
    """
    parser = argparse.ArgumentParser(
        description="Split a manifest CSV file into train, val, and test sets."
    )
    parser.add_argument(
        "--manifest-path", type=Path, required=True, help="Path to the input manifest CSV file."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        required=True,
        help="Directory where the split manifest files will be saved.",
    )
    return parser.parse_args()


def main() -> None:
    """Split the requested manifest into train, validation, and test CSV files."""
    args = parse_args()
    split_manifest(manifest_path=args.manifest_path, output_dir=args.output_dir)
