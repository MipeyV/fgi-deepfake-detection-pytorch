...
def test_create_dataloader_can_resize_frames(tmp_path: Path) -> None:
    ...
    from src.data.video import build_resize_square_transform
    ...
    dataloader = create_dataloader(
        ...
        shuffle=False,
        num_workers=0,
        frame_transform=build_resize_square_transform(4),
    )
    batch = next(iter(dataloader))
...
def test_create_dataloader_can_center_crop_resized_frames(tmp_path: Path) -> None:
    clip_path = tmp_path / "preprocessed" / "real" / "video_001" / "clips" / "000000"
    create_clip(clip_path, num_frames=3)
    ...
    from src.data.video import build_resize_center_crop_transform
